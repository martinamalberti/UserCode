...
double basicClusterCorrection (int localEta, int mod){
  
  TF1 * fData[4];
  for(int ii=0;ii<4; ii++){
    char fName[80];
    sprintf(fName, "fData_%d", ii);
    fData[ii] = new TF1(fName,"[0]+([1]*(x-0.5)-[2]*pow(x-0.5,2)-[3]*pow(x-0.5,3)-[4]*pow(x-0.5,4))",-1,1.);
  }

  fData[0]->SetParameters(0.988705,-0.0837891 , 0.0998416 , 0.00237601 , -0.0124545);
  fData[1]->SetParameters(0.987911,-0.0822352 , 0.0624357 , -0.0753827 , -0.0535641);
  fData[2]->SetParameters(0.986972,-0.0815992 , 0.00896268 , -0.197142 , -0.120907);
  fData[3]->SetParameters(0.981789,-0.0688347 , -0.201439 , -0.579391 , -0.305);
  
  double corr = fData[mod]-> Eval(localEta);
  return(1./corr);

}

double singleBasicClusterCorrection (int localEta, int mod){
  ...
  fData[0]->SetParameters(0.9895,-0.0764978 , 0.0978014 , 0.0252918 , 0.00523092);
  fData[1]->SetParameters(0.989301,-0.0787397 , 0.108515 , 0.0502772 , 0.0239275);
  fData[2]->SetParameters(0.990077,-0.0693056 , 0.0431445 , -0.0895432 , -0.0597349);
  fData[3]->SetParameters(0.986041,-0.0643558 , -0.116624 , -0.39943 , -0.21014);
...
void makeZPeakWithBCCorrection()
{
  ...
  bool useSingleBasicClusterCorrectionEta = false;
  bool useLocalCorrectionEta = true;

  //---- output file to save graphs
  char outfilename[100];
  sprintf(outfilename,"dataZpeak_allR9_BCcorr.root");
  ...
  int ele1_bcN;
  std::vector<float>   *ele1_bcE;
  std::vector<float>   *ele1_bcEta;
  std::vector<float>   *ele1_bcPhi;
  std::vector<float>   *ele1_bcLocalEta;  
  std::vector<float>   *ele1_bcLocalPhi;

  int ele2_bcN;
  std::vector<float>   *ele2_bcE;
  std::vector<float>   *ele2_bcEta;
  std::vector<float>   *ele2_bcPhi;
  std::vector<float>   *ele2_bcLocalEta;  
  std::vector<float>   *ele2_bcLocalPhi;
  ...
  ntu_Data->SetBranchAddress("ele1_fCorrection", &fCorr);  
  ntu_Data->SetBranchAddress("ele2_fCorrection", &fCorr2);  
  ...
  for(int ientry = 0; ientry < nEntries; ++ientry)
  {
    ...
    if (useLocalCorrectionEta){
      scEc* =basicClusterCorrection( scLocalEta  ,modId(scEta));
      scE2c*=basicClusterCorrection( scLocalEta2 ,modId(scEta2));
    }
    
    // corrected energy: use single BC correction
    if (useSingleBasicClusterCorrectionEta){
      for (int ibc = 0; ibc < ele1_bcN; ibc++){
	scEc+= ele1_bcE->at(ibc)*singleBasicClusterCorrection(ele1_bcLocalEta->at(ibc),modId(scEta));
      }
      scEc*=fCorr;
      for (int ibc = 0; ibc < ele2_bcN; ibc++){
	scE2c+= ele2_bcE->at(ibc)*singleBasicClusterCorrection(ele2_bcLocalEta->at(ibc),modId(scEta2));
      }
      scE2c*=fCorr2;
    }
