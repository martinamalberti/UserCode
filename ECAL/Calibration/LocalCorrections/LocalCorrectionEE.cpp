...
int main(int argc, char** argv)
{
  ...
  bool subModuleAnalysis = false;
  bool useYcoordinate = false;
  bool usePUweights = true;
  bool useR9weights = false;

  float limvar = 0.005;
  if (subModuleAnalysis) limvar = 0.025;
  
  //---- output file to save graphs
  char outfilename[100];
  sprintf(outfilename,"GraphsLocalXC_regression_EE_highR9.root");
  ...
  ntu_Data->Add("../NTUPLES/Run2011A/WZAnalysis/WZAnalysis_SingleElectron_Run2011A-WElectron-PromptSkim-v6_42XReReco_FT_R_42_V21B_new.root");
  ...
  ntu_MC->SetBranchAddress("ele1_isEEDeeGap", &isEEDeeGap);
  ...
  ntu_MC->SetBranchAddress("ele1_xS_DK",&xS_DK); 
  ntu_MC->SetBranchAddress("ele1_yS_DK",&yS_DK); 


  //---- Set branch addresses for Data
  ntu_Data->SetBranchAddress("PV_z", &PV_z);
  ntu_Data->SetBranchAddress("ele1_isEB", &isEB);
  ntu_Data->SetBranchAddress("ele1_isEEDeeGap", &isEEDeeGap);
  ...
  ntu_Data->SetBranchAddress("ele1_xS_DK",&xS_DK); 
  ntu_Data->SetBranchAddress("ele1_yS_DK",&yS_DK); 
  ...
  for(int entry = 0; entry < ntu_MC->GetEntries(); ++entry) {
    ...
    //-- removes regions near SC boundaries
    if ( fabs( xS_DK ) < 0.002 && fabs( yS_DK ) < 0.002) {
      h_template_MC[mod]-> Fill(EoP,ww);
    }

    //-- fill MC histos in x bins
    float var;
    if ( !subModuleAnalysis ) {
      var = xC_DK;
      if (useYcoordinate)  var = yC_DK;
    }

    if (subModuleAnalysis) {
      var = xS_DK;
      if (useYcoordinate)  var = yS_DK;
    }  
    
    if ( fabs(var) > limvar ) continue;
    
    int bin = nBins*(var+limvar)/(limvar*2);
    
    if (bin>nBins-1 || bin < 0 ) {
      std::cout << "Error in bins: " << bin << " " << var << std::endl;
      continue;
    }
  ...
  for(int mod=0;mod<4;mod++){
	...
	//	float xval = -limvar + (i+limvar)*1./(float)nBins;
	float xval = -limvar + (limvar*2)/nBins *i;
