// per compilare: g++ -Wall -o MomentumScaleCalibrationMod4 `root-config --cflags --glibs` MomentumScaleCalibrationMod4.cpp
...
int main(int argc, char** argv)
{
  ...
  sprintf(outfilename,"outputFiles/zeeScale_mod4_EBP.root");
  ...
  for(int entry = 0; entry < ntu_MC->GetEntries(); ++entry) {
    ...
    var = mZ/91.19;    /// use the energy for ele1
  ...
  for(int entry = 0; entry < ntu_DA->GetEntries(); ++entry) {
    ...
    //-- use the first electron
    //var  = mZ * sqrt(pTK/scEne) / 91.19;    /// use the momentum for ele1
    var = mZ/91.19;    /// use the SC energy for ele1

    //--- only mod4 
    int mod  = templIndex(ieta);
    if ( mod == selectedMod && (scEta > etaMin && scEta < etaMax ) ){
    ...
    //--- only mod4 
    mod  = templIndex(ieta2);
    if ( mod == selectedMod && (scEta2 > etaMin && scEta2 < etaMax ) ){
      (h_EoC[bin]) -> Fill(var);  // This is Data
      (h_zvtx[bin]) -> Fill( PV_z ); 
    }
  ...
  for(unsigned int i = 0; i < nBins; ++i)
  {
    ...
    //g_EoP -> SetPoint(i, zbin , pow(f_EoP[i]->GetParameter(1),2));
    g_EoP -> SetPoint(i, zbin , pow(1./f_EoP[i]->GetParameter(1),2));
    g_EoP -> SetPointError(i, ezbin, 2*f_EoP[i]->GetParError(1));
    //cout  << " ***** " <<  1./f_EoP[i]->GetParameter(1) << " " << f_EoP[i]->GetParError(1) << " " << fStatus <<  endl; 

    //ratio preparation
    float ymc  = pow(f_EoP[i]->GetParameter(1),2);
    float eymc = 2*f_EoP[i]->GetParError(1); 
    ...
    //g_EoC -> SetPoint(i, zbin, pow(f_EoC[i]->GetParameter(1),2));
    g_EoC -> SetPoint(i, zbin, pow(1./f_EoC[i]->GetParameter(1),2));
    g_EoC -> SetPointError(i, ezbin, 2*f_EoC[i]->GetParError(1));
    //    cout << " ********** " <<  1./f_EoC[i]->GetParameter(1) << " " << f_EoC[i]->GetParError(1) << endl; 

    float yda  = pow(f_EoC[i]->GetParameter(1),2);
    float eyda = 2*f_EoC[i]->GetParError(1); 
