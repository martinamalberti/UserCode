...
//     float feta = fabs(ieta);
//     if (feta <= 15)               {return 0;}
//     if (feta>  15 && feta <=  25) {return 1;}
//     if (feta>  25 && feta <=  35) {return 2;}
//     if (feta>  35 && feta <=  45) {return 3;}  
//     if (feta>  45 && feta <=  55) {return 4;}
//     if (feta>  55 && feta <=  65) {return 5;}
//     if (feta>  65 && feta <=  75) {return 6;}
//     if (feta>  75 && feta <=  85) {return 7;}
//     if (feta>  85 && feta <=  98) {return 8;}
//     if (feta>  98 && feta <= 108) {return 9;}
//     if (feta> 108 && feta <= 118) {return 10;}
//     if (feta> 118 )               {return 11;}
...
int main(int argc, char** argv)
{
  ...
  sprintf(outfilename,"outputFiles/GraphsRegionalScaleEta_lowR9_Pcalib_templatesFromMC_42XReReco_FT_R_42_V21B.root");
  ...
  //--- P calibration
  TMomentumCalibration *pCalib = new TMomentumCalibration("../CommonTools/momentumScale_scM.root"); 
  ...
  if (useW){
    ...
    // //rereco
    // ntu_Data->Add("../NTUPLES/Run2011A/WZAnalysis/WZAnalysis_SingleElectron_Run2011A-WElectron-May10ReReco-v1_42XReReco_FT_R_42_V24.root");
    // ntu_Data->Add("../NTUPLES/Run2011A/WZAnalysis/WZAnalysis_SingleElectron_Run2011A-WElectron-PromptSkim-v4_42XReReco_FT_R_42_V24.root");
    // ntu_Data->Add("../NTUPLES/Run2011A/WZAnalysis/WZAnalysis_SingleElectron_Run2011A-WElectron-PromptSkim-v5_42XReReco_FT_R_42_V24.root");
    // ntu_Data->Add("../NTUPLES/Run2011A/WZAnalysis/WZAnalysis_SingleElectron_Run2011A-WElectron-PromptSkim-v6_42XReReco_FT_R_42_V24.root");
    // ntu_Data->Add("../NTUPLES/Run2011B/WZAnalysis/WZAnalysis_SingleElectron_Run2011B-WElectron-PromptSkim-v1_42XReReco_FT_R_42_V24.root");
  ...
  histoFunc *templateHistoFuncMC[16][2]; 
  histoFunc *templateHistoFuncData[16][2]; 
  for (int mod=0; mod<Ntempl;mod++) {
    for (int ll = 0; ll < 2; ll++){
      ...
      templateHistoFuncData[mod][ll] = new histoFunc(h_template_MC[mod][ll]);
    }
  }
  ...
  for(int i = 0; i < nBins; ++i) {
    ...
    templateFunc = new TF1("templateFunc", templateHistoFuncMC[mod][0], eopmin, eopmax, 3, "histoFunc");
    templateFunc -> SetNpx(10000);
    
    xNorm = h_EoP_MC[i]->Integral()/templateHistoFuncMC[mod][0]->GetIntegral() *
            h_EoP_MC[i]->GetBinWidth(1)/h_template_MC[mod][0]->GetBinWidth(1); 
    
    templateFunc -> FixParameter(0, xNorm);
    templateFunc -> SetParameter(1, 1.01 );
    ...
    templateFunc = new TF1("templateFunc", templateHistoFuncMC[mod][1], eopmin, eopmax, 3, "histoFunc");
    templateFunc -> SetNpx(10000);

    xNorm = h_EoC_MC[i]->Integral()/templateHistoFuncMC[mod][1]->GetIntegral() *
  ...
  for(int i = 0; i < nBins; ++i)
  {
    ...
    templateFunc = new TF1("templateFunc", templateHistoFuncData[mod][0], eopmin, eopmax, 3, "histoFunc");
    templateFunc -> SetNpx(10000);
    // xNorm = h_EoP_Data[i]->Integral()/h_template_Data[mod][0]->Integral() *
    //         h_EoP_Data[i]->GetBinWidth(1)/h_template_Data[mod][0]->GetBinWidth(1); 
    xNorm = h_EoP_Data[i]->Integral()/templateHistoFuncData[mod][0]->GetIntegral() *
            h_EoP_Data[i]->GetBinWidth(1)/h_template_Data[mod][0]->GetBinWidth(1); 
    ...
    int fStatus; 
    for (int trial=0;trial<5;trial++) {
      templateFunc->SetParameter(1,1.+0.01*(trial+1));
    ...
    //if(fStatus%10 == 4)  g_EoP_Data -> SetPointError(i, 0., 10);
    //    if ( templateFunc->GetParError(1) < 0.003) g_EoP -> SetPointError(i, 0., 0.003);
    //cout  << " ***** " <<  1./templateFunc->GetParameter(1) << " " << templateFunc->GetParError(1) << endl; 
    ...
    templateFunc = new TF1("templateFunc", templateHistoFuncData[mod][1], eopmin, eopmax, 3, "histoFunc");
    templateFunc -> SetNpx(10000);

    xNorm = h_EoC_Data[i]->Integral()/templateHistoFuncData[mod][1]->GetIntegral() *
