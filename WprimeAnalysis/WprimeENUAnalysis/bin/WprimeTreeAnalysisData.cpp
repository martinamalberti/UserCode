...
int main (int argc, char** argv)
{
  ...
  for (int entry = 0; entry < nEntries; ++entry) {
    ...
    //if (treeVars.algoL1Bit[47]==0) continue;// EG5
    ...
    for (int i=0; i<treeVars.nElectrons; i++){
      ...
      if ( (fabs(eta) > EtaCutEB && fabs(eta) < EtaCutEE) || fabs(eta) > 2.5 ) continue;
    ...
    if (treeVars.eleEt[chosenEle] > 25. ) 
      cout << "Run " << treeVars.runId 
	   << "     Lumi =" << treeVars.lumiId << "   event " << treeVars.eventId 
	   << "      ET  = " << treeVars.eleEt[chosenEle] 
	   << "      ETA = " << treeVars.eleEta[chosenEle] 
	   << "      PHI = " << treeVars.elePhi[chosenEle] 
	   << "  1-E4/E1 = " << treeVars.eleSeedSwissCross[chosenEle] << endl;
    ...
    if (treeVars.eleEt[chosenEle] > 200 ){
