...
int main (int argc, char** argv)
{
  ...
  double w = Xsec*GenFilterEff*Lumi/(float)NeventsDataset;
  ...
  cout << "No selections                  " << nn         << "      " << 1.  <<endl;
  cout << "HLT Photon15 + 1 gsf ele       " << counter[1] << "      " << float(counter[1])/float(nn) <<endl;
  cout << "Electron ET > 30, eleId+iso.   " << counter[2] << "      " << float(counter[2])/float(nn) <<endl;
  cout << endl;
  
  cout << "w/o Jet Veto                   " << counter[2] << "      " << float(counter[2])/float(nn) <<endl;
  cout << "w/o Jet Veto, ET>200           " << nHighEt200 << "      " << nHighEt200/float(nn)  <<endl;
  cout << "w/o Jet Veto, ET>300           " << nHighEt300 << "      " << nHighEt300/float(nn)  <<endl;
  cout << endl;
 
  nHighEt200 = hetWithJetVeto->Integral(bin200,bin2);
  nHighEt300 = hetWithJetVeto->Integral(bin300,bin2);
 
  cout << "w/ Jet Veto                    " << counter[3] << "      " << float(counter[3])/float(nn) <<endl;
  cout << "w/ Jet Veto, ET>200            " << nHighEt200 << "      " << nHighEt200/float(nn)  <<endl;
  cout << "w/ Jet Veto, ET>300            " << nHighEt300 << "      " << nHighEt300/float(nn)  <<endl;
